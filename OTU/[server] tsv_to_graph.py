...
PATH_TO_CSV = r'/Users/owl/Pycharm/PycharmProjects/KMZ/OTU/files/out/graph.tsv'

# TODO: change node color (in server)
...
def get_big_graph(path_to_tsv):
    ...
    for index, _ in enumerate(from_list):
        pairs.append((from_list[index], to_list[index], colors[index]))
    pairs_bool = list()
    # for pair in pairs:
...
def finder(key_word, path):
    labels_list, lines_list = get_big_graph(path_to_tsv=path)
    find_id = -1
    for label in labels_list:
        if key_word.lower().strip() == label['label'].lower().strip():
            find_id = label['id']
            break
    if find_id == -1:
        return labels_list, lines_list
    else:
        mini_labels_list, mini_lines_list = mini_graph(path=path, id_need=find_id)
        return mini_labels_list, mini_lines_list
